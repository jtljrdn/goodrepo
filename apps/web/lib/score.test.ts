...
import { CATEGORIES, DEEP_SCAN_ONLY, scoreCategory, scoreRepo } from "@/lib/score"
...
test("every signal appears in exactly one category", () => {
  ...
  for (const category of CATEGORIES) {
    for (const signal of category.signals) {
      expect(
        seen.has(signal.id),
        `${signal.id} is in both ${seen.get(signal.id)} and ${category.key}`
      ).toBe(false)
...
test("a not-measured signal is excluded from both sides of the fraction", () => {
  ...
  expect(allNull.totalPoints).toBe(
    consistency.signals
      .filter(
        (s) => s.id !== "consistentRouteShape" && s.id !== "consistentErrors"
      )
...
test("a not-measured signal never lowers the score", () => {
  ...
  const measured = scoreCategory(
    consistency,
    profileWith({
      singleValidationLib: true,
      consistentNaming: true,
      singleDataLayer: true,
      lintConfig: true,
      consistentRouteShape: null,
      consistentErrors: null,
    })
...
test("the overall score averages only the categories that produced a score", () => {
  const result = scoreRepo(
    profileWith({ consistentRouteShape: null, consistentErrors: null })
  )
...
test("a deep-scan-only signal says so until a deep scan answers it", () => {
  ...
  const pending = scoreRepo(profile).categories
    .flatMap((c) => c.signals)
    .find((s) => s.id === "consistentRouteShape")
  expect(pending?.text).toContain("needs a deep scan")

  const answered = scoreRepo(profile, new Set<SignalId>(["consistentRouteShape"]))
...
test("every signal the agent can answer is marked as needing a deep scan", () => {
  for (const id of DEEP_SCAN_ONLY) {
    const signal = scoreRepo(profileWith({ [id]: null })).categories
      .flatMap((c) => c.signals)
      .find((s) => s.id === id)
