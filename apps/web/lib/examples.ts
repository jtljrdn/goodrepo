export const EXAMPLES: readonly { slug: string; sha?: string }[] = [
  ...
  { slug: "jtljrdn/goodrepo" },
]

export function exampleHref({ slug, sha }: { slug: string; sha?: string }) {
  return sha ? `/${slug}?sha=${sha}` : `/${slug}`
}
