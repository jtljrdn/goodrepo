...
test("detects a documented single-test invocation", () => {
  expect(
    detectDocs(facts({ "README.md": "`vitest run src/a.test.ts`" }), null).has
      .singleTestDocumented
  ).toBe(true)
  expect(
    detectDocs(facts({ "README.md": '`bun test -t "my case"`' }), null).has
      .singleTestDocumented
  ).toBe(true)
  expect(
    detectDocs(facts({ "README.md": "`bun test`" }), null).has
      .singleTestDocumented
  ).toBe(false)
})
