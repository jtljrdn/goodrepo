...
test("passes predictableRoot when most code sits under folders an agent can guess", () => {
  const result = detectStructure(
    facts(["src/a.ts", "src/b.ts", "src/c.ts", "src/d.ts", "weird/e.ts"]),
    []
  )
...
test("fails predictableRoot when code is scattered across top-level folders", () => {
  expect(
    detectStructure(facts(["a/x.ts", "b/y.ts", "c/z.ts", "d/w.ts"]), []).has
      .predictableRoot
  ).toBe(false)
})

test("measures depth and applies the threshold at the boundary", () => {
  expect(detectStructure(facts(["a/b/c/d/e/f/g.ts"]), []).has.shallowTree).toBe(
    true
  )
  expect(
    detectStructure(facts(["a/b/c/d/e/f/g/h.ts"]), []).has.shallowTree
  ).toBe(false)
})

test("passes colocatedTests when tests sit beside their source", () => {
  const result = detectStructure(
    facts(["src/a.ts", "src/a.test.ts", "src/b.ts", "src/b.test.ts"]),
    []
  )
...
test("fails generatedExcluded when build output is committed", () => {
  expect(
    detectStructure(facts(["src/a.ts", "dist/a.js"]), []).has.generatedExcluded
  ).toBe(false)
  expect(detectStructure(facts(["src/a.ts"]), []).has.generatedExcluded).toBe(
    true
  )
})
...
test("a build/ directory holding source is not treated as build output", () => {
  expect(
    detectStructure(facts(["src/a.ts", "build/build.ts"]), []).has
      .generatedExcluded
  ).toBe(true)
  expect(
    detectStructure(facts(["src/a.ts", "out/page.ts"]), []).has
      .generatedExcluded
  ).toBe(true)
})
...
test("workspace roots count as predictable in a monorepo", () => {
  const paths = [
    "apps/web/a.ts",
    "apps/web/b.ts",
    "packages/ui/c.ts",
  ...
  expect(detectStructure(facts(paths), []).has.predictableRoot).toBe(false)
  expect(
    detectStructure(facts(paths), ["apps", "packages", "tools"]).has
      .predictableRoot
  ).toBe(true)
})

test("root-level config files do not count as scattered source", () => {
  const result = detectStructure(
    facts(["eslint.config.js", "next.config.ts", "src/a.ts", "src/b.ts"]),
    []
  )
  expect(result.measurements.predictableRoot?.value).toBe(1)
})

test("depth is judged at the 90th percentile, so one deep fixture folder does not fail the repo", () => {
  const shallow = Array.from({ length: 9 }, (_, i) => `src/a/f${i}.ts`)
  const result = detectStructure(
    facts([...shallow, "src/fixtures/a/b/c/d/e/f/g/h.ts"]),
    []
  )
  expect(result.has.shallowTree).toBe(true)
  expect(result.maxDirectoryDepth).toBe(10)
})
