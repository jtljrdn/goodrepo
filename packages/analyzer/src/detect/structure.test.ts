...
test("passes predictableRoot when most code sits under one top-level folder", () => {
  const result = detectStructure(
    facts(["src/a.ts", "src/b.ts", "src/c.ts", "src/d.ts", "scripts/e.ts"])
  )
...
test("measures depth and applies the threshold at the boundary", () => {
  ...
  expect(detectStructure(facts(["a/b/c/d/e/f/g/h.ts"])).has.shallowTree).toBe(
    false
  )
})
...
test("a build/ directory holding source is not treated as build output", () => {
  expect(
    detectStructure(facts(["src/a.ts", "build/build.ts"])).has.generatedExcluded
  ).toBe(true)
  expect(
    detectStructure(facts(["src/a.ts", "out/page.ts"])).has.generatedExcluded
  ).toBe(true)
})
